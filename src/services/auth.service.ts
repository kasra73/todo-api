...
class AuthService {
  ...
  public async login(userData: CreateUserDto): Promise<{ tokenData: TokenData; findUser: User }> {
    if (isEmpty(userData)) throw new HttpException(400, 'UserData cannot be empty');
  ...
  public async logout(userData: User): Promise<User> {
    ...
    if (findUser === null) throw new HttpException(409, `You're email ${userData.email} not found`);
